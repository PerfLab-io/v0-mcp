...
async function testStreamableHTTP() {
  ...
  console.log(
    "This demonstrates SSE streams within HTTP responses as per MCP specification.\n",
  );

  try {
    console.log(
      "1. Sending streamable HTTP request with Accept: text/event-stream...",
    );
        ...
        for (const line of lines) {
          if (line.startsWith("data: ")) {
            ...
            try {
              ...
              if (parsed.method === "notifications/message") {
                ...
                if (parsed.params.data.newLevel) {
                  console.log(
                    `   ⚙️  New Log Level: ${parsed.params.data.newLevel}`,
                  );
                }
                ...
                console.log(
                  `   📋 Result: ${JSON.stringify(parsed.result || parsed.error, null, 6)}`,
                );
            ...
            if (receivedFinalResponse || eventCount >= 10) {
              console.log(
                "✅ Received final response or enough events, disconnecting...",
              );
              controller.abort();
              break;
            }
