...
export class RateLimiter {
  ...
  async getRateLimitInfo(
    sessionIds: string[],
    windowMs: number = 60000,
  ): Promise<Record<string, RateLimitState>> {
    ...
    try {
      ...
      console.error(
        "Failed to get rate limit info for multiple sessions:",
        error,
      );
  ...
  async cleanup(olderThanMs: number = 3600000): Promise<number> {
    ...
    try {
      ...
      if (results) {
        cleanedCount = results.reduce(
          (total: number, result) => total + ((result as number) || 0),
          0,
        );
      }
