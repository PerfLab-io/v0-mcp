...
const SENSITIVE_PATTERNS = {
  ...
  credentials:
    /(api[\s_-]?key|token|secret|password|auth|bearer|jwt|session[_-]?id)[\s:="'`]+(\S+)/gi,
  ...
  privateKey:
    /-----BEGIN\s+(PRIVATE\s+KEY|RSA\s+PRIVATE\s+KEY)-----[\s\S]*?-----END\s+(PRIVATE\s+KEY|RSA\s+PRIVATE\s+KEY)-----/gi,
};
...
export function createSafeLogData(originalData: any): any {
  try {
    ...
    return {
      ...
      message:
        "Sensitive data filtering failed - original data redacted for safety",
    };
  }
}
