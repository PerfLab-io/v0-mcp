...
describe('MCPLogger', () => {
  ...
  test('log respects session minimum level', async () => {
    ...
    const debugResult = await logger.log('session1', LogLevel.DEBUG, 'test', { msg: 'debug' });
    expect(debugResult).toBe(false);

    // ERROR message should be logged (higher priority than WARNING)
    const errorResult = await logger.log('session1', LogLevel.ERROR, 'test', { msg: 'error' });
  ...
  test('log respects rate limiting', async () => {
    ...
    const result = await logger.log('session1', LogLevel.INFO, 'test', { msg: 'info' });
  ...
  test('handles errors gracefully', async () => {
    ...
    const result = await logger.log('session1', LogLevel.INFO, 'test', { msg: 'test' });
    expect(result).toBe(false);
  });
});
...
describe('Integration', () => {
  test('complete logging flow works end-to-end', async () => {
    ...
    const debugResult = await logger.log('session1', LogLevel.DEBUG, 'test', { msg: 'debug' });
    const infoResult = await logger.log('session1', LogLevel.INFO, 'test', { msg: 'info' });
    const errorResult = await logger.log('session1', LogLevel.ERROR, 'test', { msg: 'error' });
